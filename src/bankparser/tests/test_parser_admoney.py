...
class ADMoneyTest(unittest.TestCase):
    ...
    parser_admoney = Parser.StatementParser(bank2, sampletxt2)
    # def setUp(self):
    #     self.parser = Parser.StatementParser(self.bank, self.sampletxt)

    def test_bankname(self):
        self.assertEqual(self.parser_admoney.bank,self.bank,'Bank name in parser')

    def test_bankname2(self):
        self.assertEqual(self.parser_admoney.statement.bank,self.bank,'Bank name in statement')

    def test_account(self):
        self.assertEqual(self.parser_admoney.statement.account, '12345-000','account in statement')

    def test_line1amount(self):
        amount = self.parser_admoney.statement.lines[0].amount
        self.assertEqual(amount, 721.84, 'Amount in first line (+)')

    def test_line2amount(self):
        amount = self.parser_admoney.statement.lines[1].amount
        self.assertEqual(amount, -0.44, 'Amount in second line (-)')

    def test_linedescr(self):
        descr = self.parser_admoney.statement.lines[0].description
